(() => {
  ...
  function buildPages(state) {
    ...
    return [
      ...
      page(9, '웃음빛', `토끼가 풍선 속으로 쏙 들어가자 ${name}과 토끼는 한참을 깔깔 웃었어요.`, `${worldArt(state.worldTheme).replace('</div>', `${avatar(state, 'pose-cheer', 'avatar-pos-left')}</div>`)}`, `반짝, ${name}의 마음에서 웃음빛이 피어났어요.`),
      ...
      page(11, '길을 잃은 아기곰', '어두운 터널 한쪽에서는 길을 잃은 아기곰이 훌쩍이고 있었어요.', `<div class="page-art scene-night"><div class="tunnel"></div><div class="bear-emoji">🐻</div><div class="tear">💧</div>${avatar(state, '', 'avatar-pos-right')}</div>`, '집으로 가는 길을 모르겠어…'),
      page(12, '다정한 손', `${name}은 아기곰에게 손을 내밀었어요. “괜찮아. 같이 길을 찾아보자.”`, `<div class="page-art scene-warm"><div class="kind-glow"></div><div class="bear-emoji">🐻</div>${avatar(state, '', 'avatar-pos-right')}</div>`),
      ...
      page(15, '한 걸음씩', `조금 무서웠지만 여우가 힘차게 응원했어요. “${name}, 천천히 한 걸음씩 올라가면 돼!”`, `<div class="page-art scene-night"><div class="ladder"></div><div class="switch"></div><div class="fox-emoji">🦊</div>${avatar(state, 'pose-climb', 'avatar-pos-right')}</div>`, '무서워도 한 걸음씩.'),
      ...
      page(19, '별빛 생일역', '생일열차가 반짝이는 역에 멈췄어요. 문이 열리자 토끼와 아기곰, 여우가 먼저 달려 나갔어요.', `<div class="page-art scene-station"><div class="station-sign">별빛 생일역</div><div class="station-platform"></div><div class="station-balloons">🎈🎈</div><div class="animal-line">🐰🐻🦊</div></div>`),
      ...
      page(21, '생일 축하해!', `동물 친구들은 풍선을 흔들고 별콩 차장은 작은 종을 울렸어요. 케이크 옆에는 ${gift}도 놓여 있었지요.`, `<div class="page-art scene-warm"><div class="gift-box">🎁</div><div class="cake"></div><div class="animal-line">🐰🐻🦊</div></div>`, `${name}, 생일 축하해!`),
